...
export async function getMobileSqlite(randomString: string, vault: string): Promise<Store> {
  const updateTable = `"updates${randomString}-${vault}"`;
  const pullTable = `"pulls-${randomString}-${vault}"`;
  const dbName = `db-${randomString}-${vault}`;
